...
function W:neighbors(resource)
	...
	local function filter_action(action)
		if action.type == "travel" then
			...
			if Luggage.lock(resource) then
				return Luggage.unlocked(resource)
			else
				return true
			end
		end
...
function W:has(resource, count)
	return A.Game.Luggage.has(resource, count, { include_bank = true, include_inventory = true })
end

function W:count(resource)
	return A.Game.Luggage.count(resource, { include_bank = true, include_inventory = true })
end
