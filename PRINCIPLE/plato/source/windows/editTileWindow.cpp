...
plato::Application::EditTileWindow::EditTileWindow(Application* application) :
	principle::Window(application)
{
	this->application = application;
}

void plato::Application::EditTileWindow::close()
{
	this->tiles.clear();
	principle::Window::close();
}

std::string plato::Application::EditTileWindow::get_title() const
{
	return "Edit Tile";
}

void plato::Application::EditTileWindow::set_tile(
...
void plato::Application::EditTileWindow::add_tile(
	const thoreau::TileCoordinate& tile_coordinate)
{
	auto& map = this->application->get_game_manager().get_map();
	if (!map.has_tile(tile_coordinate))
...
void plato::Application::EditTileWindow::update_gui()
{
	...
	ImGui::Indent();
	{
		bool tile_portal_up = this->current_flags & thoreau::Tile::flag_portal_up;
		if (ImGui::Checkbox("portal_up", &tile_portal_up))
		{
			flags |= thoreau::Tile::flag_portal_up;
		...
		bool tile_portal_down = this->current_flags & thoreau::Tile::flag_portal_down;
		if (ImGui::Checkbox("portal_down", &tile_portal_down))
		{
			flags |= thoreau::Tile::flag_portal_down;
...
void plato::Application::EditTileWindow::set_value(
