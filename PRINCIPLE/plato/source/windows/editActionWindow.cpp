...
plato::Application::EditActionWindow::EditActionWindow(Application* application) :
...
plato::Application::EditActionWindow::EditActionWindow(
...
void plato::Application::EditActionWindow::add(const twoflower::Action& action)
{
	this->actions.push_back(action);
	open();
}

void plato::Application::EditActionWindow::draw()
{
	if (get_is_open())
	{
...
void plato::Application::EditActionWindow::close()
{
	this->application->remove_window(this);
	principle::Window::close();
}

int plato::Application::EditActionWindow::get_flags() const
{
	return ImGuiWindowFlags_NoSavedSettings;
}

std::string plato::Application::EditActionWindow::get_title() const
{
	return "Edit Action";
}

std::string plato::Application::EditActionWindow::get_id() const
{
	return principle::Window::get_id() + "@" + std::to_string(this->id);
}

void plato::Application::EditActionWindow::update_gui()
{
	for (std::size_t i = 0; i < this->actions.size(); ++i)
	{
		this->application->brochure_widget_manager->edit(this->actions[i]);
